...
\title{\textbf{Camera-in-the-Loop Optical Training of a\\ Physical Diffractive Network on a Smartphone}\\[4pt]
{\large Svetoch Series, Paper VI of VI}}
\author{\svetochauthor}
\date{17 June 2026\\[2pt]{\normalsize Published on Zenodo --- DOI \href{https://doi.org/10.5281/zenodo.20730393}{10.5281/zenodo.20730393}}}
...
close the loop and \textbf{train}. We treat the optical channel itself --- the smartphone's
display$\to$mirror$\to$camera path, a \textbf{single-layer physical diffractive network} whose
forward pass is carried by near-field (Talbot) diffraction and on-sensor integration --- as the
layer under optimization: the weights are displayed as brightness, the camera reads the channel's true
...
We call the trainable element a \emph{physical diffractive network}: light from the displayed
...
training. We mean this in the literal, single-layer sense --- \textbf{not} a multilayer
diffractive deep neural network (D\textsuperscript{2}NN) in the sense of Lin \emph{et al.}; the
distinction is made explicit in Section~7.

\section{The optical channel as a physical diffractive network}
The trainable layer is the screen$\to$mirror$\to$camera channel of Paper~I. Light leaving the
...
is a single physical diffractive layer: a fixed optical transform whose \emph{inputs} (the
displayed weights) we adjust by training, rather than a stack of learned phase masks.
...
the learner sees the true optics. \textbf{Scope of the diffractive-network claim:} the trainable
element is a \emph{single} physical diffractive layer (the display$\to$mirror$\to$camera channel),
and what is optimized is the displayed input, not a stack of learned diffractive masks --- this is
deliberately weaker than a multilayer diffractive deep neural network (D\textsuperscript{2}NN,
Lin \emph{et al.}); no D\textsuperscript{2}NN claim is made. \textbf{Future:} a fully optical
update (accumulating the inverted-overlay residual back onto the weights), optical batch
normalization, stacking several diffractive layers toward a genuine multilayer optical network,
and a trained optical MLP.
